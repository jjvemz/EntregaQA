...
export default function RegisterForm() {
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().matches(/^[a-zA-Z]*$/, "Ingrese su nombre correctamente").required("Tu nombre es obligatorio"),
      lastname: Yup.string().matches(/^[a-zA-Z]*$/, "Ingrese su apellido correctamente").required("Tu apellido es obligatorio"),
      rut: Yup.string().matches(/^\d{1,2}\d{3}\d{3}[0-9kK]{1}$/, "Ingrese el rut sin puntos ni guion").required("Ingrese el rut sin puntos ni guion"),
